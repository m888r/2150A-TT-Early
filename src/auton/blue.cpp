...
namespace auton {
  void blueProtectedEight() {
    ...
    timer.placeMark();
    while (std::abs(robot::lift.getPosition()) > 10 &&
          timer.getDtFromMark() < 750_ms) {
      pros::delay(10);
    }
    rd4b::moveTarget(1250, 200);  // was 1250
    timer.placeMark();
    ...
    tray::changeMode(tray::mode::standby);
    Async({
      drive::moveTo(Pose(3.9_ft, -0.4_ft, -30_deg), 2.4_in, std::nullopt, std::nullopt, std::nullopt, PIDGains(5.3, 0, 0), PIDGains(1.5, 0, 0)); // position 4
    ...
    rd4b::changeState(rd4b::state::resetting);
    Async({
      drive::moveTo(Pose(1.5_ft, -1.72_ft, -135_deg), 4_in, 2_deg, std::nullopt, std::nullopt, PIDGains(5.3, 0, 0), PIDGains(1.4, 0, 0), std::nullopt, 0.12); // position 5
    })
    ...
    Async({
      drive::moveTo(Pose(-0.35_ft, -1.87_ft, -90_deg), 5_in, std::nullopt,
